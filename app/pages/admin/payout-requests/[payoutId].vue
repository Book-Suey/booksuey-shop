...
<template>
  <section class="admin-page">
    ...
    <template v-else>
      ...
      <template v-else-if="data.payoutRequest.status === 'disbursing'">
        <article class="vendor-panel stack-grid">
          <div>
            <h2>Manual provider recheck</h2>
            <p class="panel-copy">
              This payout is still in disbursing status. Trigger a manual
              provider reconciliation to sync the latest PayPal status.
            </p>
          </div>
          ...
          <div class="stack-grid">
            <button
              ...
              @click="handleRecheckDisbursement"
            >
              {{
                isRechecking
