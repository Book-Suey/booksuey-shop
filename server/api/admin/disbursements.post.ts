...
export default defineEventHandler(async (event) => {
  ...
  const result = await runWithOptionalTransaction(async (session) => {
    const payoutRequest = await PayoutRequest.findOne({ payoutRequestId: payload.payoutRequestId }, undefined, { session })
  ...
  const balance = await recomputeBalanceSnapshot(result.payoutRequest.vendorId)
